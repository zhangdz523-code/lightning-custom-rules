...
import acr.browser.lightning.adblock.custom.CustomRule
import acr.browser.lightning.adblock.custom.CustomRulesRepository
...
class AdBlockSettingsScreen @Inject constructor(
    ...
    private val customRulesRepository: CustomRulesRepository,
) {
    fun createSettingsFrameworkState(): SettingsFrameworkState = SettingsFrameworkState(
        resourceProvider.stringResource(R.string.settings_adblock), content = listOf(
            ...
            ClickableState(
                enabled = { userPreferencesDataStore.adBlockEnabled.get() },
                title = resourceProvider.stringResource(R.string.custom_rule_add),
                summary = { resourceProvider.stringResource(R.string.custom_rule_add_description) },
                onClick = ClickableOnClick.Input(
                    produceState = {
                        SettingsBottomSheetInputState(
                            title = resourceProvider.stringResource(R.string.custom_rule_add),
                            hint = resourceProvider.stringResource(R.string.custom_rule_hint),
                        )
                    },
                    onValueUpdated = { value ->
                        ...
                        if (pattern.isEmpty() || !pattern.contains(".") && !pattern.contains("*")) {
                            ClickableOnClick.Snackbar {
                                SettingsSnackBarState(
                                    resourceProvider.stringResource(R.string.custom_rule_invalid)
                                )
                            }
                        } else {
                            ClickableOnClick.Action {
                                customRulesRepository.addRule(CustomRule(pattern = pattern))
                            }
                        }
                    }
                )
            ...
            ClickableState(
                enabled = { userPreferencesDataStore.adBlockEnabled.get() },
                title = resourceProvider.stringResource(R.string.custom_rule_clear_all),
                summary = { resourceProvider.stringResource(R.string.custom_rule_clear_all_description) },
                onClick = ClickableOnClick.Action {
                    customRulesRepository.removeAllRules()
                }
            )
        )
    )
